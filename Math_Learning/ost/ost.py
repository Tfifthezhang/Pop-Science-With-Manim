...
class HousePrice(ThreeDScene):
    ...
    def trends(self):
        with open('ost_data.json', 'r', encoding='utf-8') as file:
            city_data = json.load(file)
        # 16个城市的房价均在下降
        vg_ax = VGroup(*[CommonFunc.add_axes(x_range=[0, 6, 18], y_range=[0, 19, 5], x_length=8, y_length=4,
                                             axis_config={"include_tip": False, "include_numbers": False}) for _ in
                         range(16)])

        vg_ax.arrange_in_grid(4, 4, buff=0.4).scale(0.35)

        vg_path = VGroup()
        vg_dot = VGroup()

        for i in range(16):
            inital_coord = city_data[i]['prices'][0] / 10000
            dot = Dot(vg_ax[i].c2p(0, inital_coord), radius=DEFAULT_DOT_RADIUS, color=RED).scale(0.7)
        ...
        for r in range(1, 29):
            self.play(
                *[vg_dot[k].animate.move_to(vg_ax[k].c2p(r / 5, city_data[k]['prices'][r] / 10000)) for k in range(16)])

        self.wait(1)

        self.vg_target = VGroup(vg_ax[9], vg_dot[9], vg_path[9])

    def policy_sell(self):
        self.move_camera(frame_center=self.vg_target, zoom=4)
        self.wait(2)


class HouseDis(Scene):
    def construct(self):
        self.ordinary_policy_sell()

    def ordinary_policy_sell(self):
        # 普通策略：价格低，我比他低一点就能卖出去
        with open('ost_data.json', 'r', encoding='utf-8') as file:
            city_data = json.load(file)

        data_example = np.array(city_data[9]['prices']) / 3
        data_example = list(map(int,data_example))
        inital_coord = data_example[0] / 10000

        ax = CommonFunc.add_axes(x_range=[0, 6, 18], y_range=[0, 6, 6], x_length=8, y_length=6,
                                 axis_config={"include_tip": False, "include_numbers": False})
        dot = Dot(ax.c2p(0, inital_coord), radius=DEFAULT_DOT_RADIUS, color=MAROON)
        path = TracedPath(dot.get_center, color=YELLOW)

        price = Variable(var=int(data_example[0]),
                         label=Text('sale price').scale(0.7),
                         var_type=Integer,
                         color=RED).scale(0.7).next_to(dot, RIGHT)
        price.add_updater(lambda m: m.next_to(dot, RIGHT))
        ...
        # 第四阶段，继续贴着成交价
        listing_data = []
        for i in list(data_example):
            if i > 58000:
                listing_data.append(60100)
                continue
            if 34000 < i < 36000:
                listing_data.append(i+2000)
                continue
            listing_data.append(i + 100)
        listing_dot = Dot(ax.c2p(0, listing_data[0] / 10000), radius=DEFAULT_DOT_RADIUS, color=YELLOW)
        list_path = TracedPath(listing_dot.get_center, stroke_color=YELLOW)
        listing_price = Variable(var=int(listing_data[0]),
                                 label=Text('卖家').scale(0.6),
                                 var_type=Integer,
                                 color=YELLOW).scale(0.7).next_to(listing_dot, UP)
        listing_price.add_updater(lambda m: m.next_to(listing_dot, UP))

        self.play(Create(listing_dot))

        self.add(list_path)
        self.wait(1)

        self.play(Write(listing_price))
        # 买方心态，要在成交价上往下，甚至是在低价位保持不动
        # 第一阶段，贴着成交价，往下走走
        # 第二阶段，看空楼市
        # 第三阶段，以为房价稳定了，会去贴着成交价往下走走
        # 第四阶段，会更加看空楼市
        target_data = []
        for n,i in enumerate(data_example):
            if i > 58000:
                target_data.append(i-1000*(1+n/10))
                continue
            if 34000 < i < 36000:
                target_data.append(int(i)-2000*(1+n/10))
                continue
            target_data.append(int(i)-3000*(1+n/10))

        target_dot = Dot(ax.c2p(0, target_data[0] / 10000), radius=DEFAULT_DOT_RADIUS, color=GREEN)
        target_path = TracedPath(target_dot.get_center, stroke_color=GREEN)
        target_price = Variable(var=int(target_data[0]),
                                 label=Text('买家').scale(0.6),
                                 var_type=Integer,
                                 color=GREEN).scale(0.7).next_to(target_dot, DOWN)
        target_price.add_updater(lambda m: m.next_to(target_dot, DOWN))

        self.play(Create(target_dot))

        self.add(target_path)
        self.wait(1)

        self.play(Write(target_price))

        for r in range(1, 29):
            self.play(dot.animate.move_to(ax.c2p(r / 5, data_example[r] / 10000)),
                      price.tracker.animate.set_value(int(data_example[r])))
            self.play(listing_dot.animate.move_to(ax.c2p(r / 5, listing_data[r] / 10000)),
                      listing_price.tracker.animate.set_value(int(listing_data[r])))
            self.play(target_dot.animate.move_to(ax.c2p(r / 5, target_data[r] / 10000)),
                      target_price.tracker.animate.set_value(int(target_data[r])))

            if r == 4 or r == 15 or r == 21:
                self.wait()
        ...
        self.play(FadeIn(bounded_rects))
        self.wait(1)

class Policy_Anly(Scene):
    def construct(self):
        self.ax = None
        self.graph = VGroup()
        self.area = None
    ...
    def policy_anly(self):
        ...
        graph1 = ax.plot(lambda x: data_example[int(x*5)] / 10000, x_range=[0, 5.1, 0.2], color = WHITE)
        graph_listing = ax.plot(lambda x: listing_data[int(x*5)] / 10000, x_range=[0, 5.1, 0.2], color = YELLOW)
        graph_target = ax.plot(lambda x: target_data[int(x*5)] / 10000, x_range=[0, 5.1, 0.2], color = GREEN)
        ...
        ## 如果你手里持有一套房产，并且认为房价还有很大的下降空间，那么就应该早点卖了，而不是犹豫不决，首鼠两端
        init_n = 0
        listing_dot = Dot(ax.c2p(init_n, listing_data[init_n*5] / 10000), radius=DEFAULT_DOT_RADIUS, color=MAROON)
        self.play(MoveAlongPath(listing_dot, graph_listing, rate_func=linear, run_time=2))
        ...
        bounded_rects = ax.get_riemann_rectangles(
            graph_listing,
            bounded_graph=graph_target,
            dx=0.05,
            x_range=[0.1, 5.1],
            show_signed_area=False,
            color=(GREY_A, GRAY_A, GREY_C))

        self.play(Create(bounded_rects))
        self.wait(1)

        inter_area = ax.get_area(graph_listing, x_range=(0.01, 5.1), bounded_graph=graph_target, color=GRAY, opacity=0.7)

        self.play(FadeTransform(bounded_rects, inter_area))

        self.wait(1)

        ## 只有等到这两条线的差距，也就是面积的增长率越来越小，才说明房价开始平稳，但目前来看，并没有这个迹象
        fu_listing = ax.plot(lambda x: listing_data[int(x * 5)] / 10000, x_range=[5.1, 5.6,0.2],color=WHITE)
        fu_target = ax.plot(lambda x: target_data[int(x * 5)] / 10000, x_range=[5.1, 5.6, 0.2], color=WHITE)

        inter_area = ax.get_area(fu_listing, x_range=(5.1, 5.6), bounded_graph=fu_target, color=GRAY,
                                 opacity=0.2)

        self.play(FadeIn(fu_listing, target_position=graph_listing.get_end()),
                  FadeIn(fu_target, target_position=graph_target.get_end()),
                  FadeIn(inter_area))

        self.wait(2)

        ## 所以唯一的策略就是降价，降价也是有策略的
        ### 回顾之前的操作，就是个不断后悔的过程，后悔自己没有降到合适的价位，结果价格不断下跌
        n = 1
        listing_dot_2 = Dot(ax.c2p(n, listing_data[n*5] / 10000), radius=DEFAULT_DOT_RADIUS, color=YELLOW)
        target_dot_2 = Dot(ax.c2p(n, target_data[n*5] / 10000), radius=DEFAULT_DOT_RADIUS, color=GREEN)
        t_label = ax.get_T_label(x_val=n, graph=graph_listing, label=Tex(""), label_color=BLUE)
        self.play(Write(t_label),
                  Create(listing_dot_2))
        self.wait(1)
        self.play(FadeIn(target_dot_2, target_mobject=listing_dot_2))
        self.wait(1)
        self.play(t_label.animate.become(ax.get_T_label(x_val=2.2, graph=graph_listing, label=Tex(""), label_color=BLUE)),
                  listing_dot_2.animate.become(Dot(ax.c2p(2.2, listing_data[11] / 10000), radius=DEFAULT_DOT_RADIUS, color=YELLOW)),
                  target_dot_2.animate.become(Dot(ax.c2p(2.2, target_data[11] / 10000), radius=DEFAULT_DOT_RADIUS, color=GREEN)))

        self.wait(1)
        self.play(t_label.animate.become(ax.get_T_label(x_val=4.2, graph=graph_listing, label=Tex(""), label_color=BLUE)),
                  listing_dot_2.animate.become(Dot(ax.c2p(4.2, listing_data[21] / 10000), radius=DEFAULT_DOT_RADIUS, color=YELLOW)),
                  target_dot_2.animate.become(Dot(ax.c2p(4.2, target_data[21] / 10000), radius=DEFAULT_DOT_RADIUS, color=GREEN)))

        self.wait(1)
        ### 但具体该怎么降呢，但麻烦的是，你不知道要降到什么程度
        ### 我们想降到一个合理的价位来达成交易，但我们不知道买家的心理价位，所以就需要样本去试探着得知，来达成交易
        self.play(t_label.animate.become(ax.get_T_label(x_val=5.1, graph=graph_listing, label=Tex(""), label_color=BLUE)),
                  FadeOut(listing_dot_2), FadeOut(target_dot_2))
        self.wait(1)

        self.play(listing_dot.animate.shift(2*DOWN))
        self.play(listing_dot.animate.shift(1.5*UP))

        self.wait(1)

        ## 至少要有一定量感兴趣的人，一方面可以让他们形成竞争，另一方面也方便用策略
        svg_people = SVGMobject('svg_icon/people.svg', fill_color=BLUE).scale(0.7)
        svg_group = VGroup(*[svg_people.copy() for _ in range(20)])
        svg_group.arrange_in_grid(rows=4, cols=5).next_to(listing_dot,2*UP)

        # dot = Dot(ax.c2p(1, graph_target[5] / 10000-100), radius=DEFAULT_DOT_RADIUS, color=MAROON)
...
class OSTCalculus(ThreeDScene):
    ...
    def calculus_anly(self):
        ...
        # path_BA = ParametricFunction(self.func2, t_range=[1, 0.3678794], color=YELLOW)
        graph_BA = chart_res[0].plot(self.func2, x_range=[1, 0.3678794], use_smoothing=True, color=YELLOW)
        ...
        tangent_line.add_updater(
            lambda m: m.become(chart_res[0].get_secant_slope_group(x=chart_res[0].p2c(moving_dot.get_center())[0],
                                                                   graph=chart_res[1],
                                                                   dx=0.001,
                                                                   secant_line_length=1.3,
                                                                   secant_line_color=RED_D)))
        ...
        derivative_value.add_updater(
            lambda m: m.tracker.set_value(self.derivative_func(chart_res[0].p2c(moving_dot.get_center())[0])))
        ...
        self.play(moving_dot.animate.move_to(chart_res[0].c2p(0.3678794, self.func2(0.3678794))))

        ## 一阶导数为零
        lines = chart_res[0].get_lines_to_point(moving_dot.get_center(), color=GREEN)
        local_label = MathTex('P^{\prime} = -(\ln(x)+1)=0').scale(0.5).next_to(moving_dot, 1.5 * UP)
        coord_tex = MathTex('(\\frac{1}{\mathrm{e}},\\frac{1}{\mathrm{e}})', color=YELLOW).scale(0.7).next_to(
            local_label, RIGHT)

        self.play(Create(lines))
        self.wait(1)
        self.play(FadeTransform(derivative_value, local_label))
        self.wait(1)
        self.play(FadeIn(coord_tex, target_position=local_label))
        self.wait(1)
