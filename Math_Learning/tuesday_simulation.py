...
class ThusdaySim(Scene):
    ...
    def processing(self):
        ...
        for i in range(100):
            ...
            self.play(Transform(self.c_label, chart2.get_bar_labels(font_size=48)))
        print(d_chart1_value, d_chart2_value)
        # self.wait(1)

        answer = MathTex("\\frac{6}{15}").scale(1)
        cal_P = MathTex("=0.400").scale(0.8).next_to(answer, 1.2 * RIGHT)

        c_label_sex = chart1.get_bar_labels(font_size=48)
        self.play(Write(c_label_sex))
        self.wait(1)
...
class BarGen(Scene):
    ...
    def table_dis(self):
        np.random.seed(9)
        values_sex = [25, 24, 24, 27]
        chart1 = BarChart(
            values_sex,
            bar_names=['bb', 'bg', 'gb', 'gg'],
            y_range=[0, 300000, 100000],
            y_length=6,
            x_length=10,
            x_axis_config={"font_size": 50})
        ...
        chart2 = BarChart(
            ...
            bar_names=['One of them is a boy, born on Tuesday',
                       'The remaining one is a boy'],
            y_range=[0, 150000, 50000],
            y_length=6,
            x_length=10,
            x_axis_config={"font_size": 30})

        vg_chart = VGroup(chart1, chart2).arrange_submobjects(RIGHT, buff=0.2).scale(0.6)
        self.play(Create(vg_chart))
        self.wait(2)

        self.vg_chart = vg_chart

        c_bar_lbls1 = chart1.get_bar_labels(font_size=24,buff=0.5)
        c_bar_lbls = chart2.get_bar_labels(font_size=24, buff=0.5)

        self.play(Write(c_bar_lbls1),
                  Write(c_bar_lbls))
        self.wait(1)

        self.c_label = VGroup(c_bar_lbls1, c_bar_lbls)

        n_var = Variable(100, MathTex('N'), var_type=Integer)
        p_var = Variable(0.4, MathTex('P'), num_decimal_places=5)

        vg_var = VGroup(n_var, p_var).scale(0.8).arrange_submobjects(DOWN, buff=0.5).next_to(vg_chart, UP)
    ...
    def barloop(self):
        ...
        N,P = self.var
        chart1, chart2 = self.vg_chart
        lable1, label2 = self.c_label

        for n in [1000,5000,10000,20000,30000,50000,80000,100000,200000,400000,500000,800000,1000000]:
            a_week = np.random.randint(low=0, high=7, size=(n, 2))
            a_sex = np.random.randint(0, 2, size=(n, 2))
            self.play(N.tracker.animate.set_value(n))
            d_chart1_value = {'0': 0, '1': 0, '2': 0, '3': 0}
            d_chart2_value = {'O': 0, 'T': 0}
            for i in range(n):
                l_week = a_week[i]
            ...
            self.play(chart1.animate.change_bar_values(list(d_chart1_value.values())),
                      chart2.animate.change_bar_values(list(d_chart2_value.values())))
            self.play(Transform(lable1, chart1.get_bar_labels(font_size=24,buff=0.5,color=WHITE)),
                      Transform(label2, chart2.get_bar_labels(font_size=24,buff=0.5,color=WHITE)))
            self.play(P.tracker.animate.set_value(d_chart2_value['T']/d_chart2_value['O']))
            print(d_chart1_value, d_chart2_value)

        std_res = MathTex("\\frac{13}{27}", "\\approx", "0.48148").scale(0.8).next_to(self.var, LEFT)
...
class GeoSim(Scene):
    ...
    def display(self):
        ...
        for i in range(100):
            ...
            y = a_y[i]
            sex = a_sex[i]
            dot1 = Dot(ax.c2p(x[0], y[0]), radius=1 * DEFAULT_DOT_RADIUS, stroke_width=8, fill_opacity=.5,
                       color=d_sex_color[a_sex[i][0]])
            dot2 = Dot(ax.c2p(x[1], y[1]), radius=1 * DEFAULT_DOT_RADIUS, stroke_width=8, fill_opacity=.5,
                       color=d_sex_color[a_sex[i][1]])
...
class GeoGen(ThreeDScene):
    ...
    def construct(self):
        ...
        math_res = MathTex("\\frac{10}{16}+", "\\frac{8}{19}+", "\\frac{10}{18}+", "\\frac{10}{15}+", "\\frac{8}{13}+",
                           "\\frac{5}{16}+", "\\frac{7}{17}+", "\\frac{6}{14}+", "\\frac{3}{12}+",
                           "\\frac{9}{17}").scale(0.55).next_to(vg_scene, 0.1 * DOWN)
