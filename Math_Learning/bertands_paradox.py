...
class Introduction(MovingCameraScene):
    def construct(self):
        ...
        alternate_glow = VGroup(
            *[Line(ORIGIN, RIGHT / 4, color=YELLOW, stroke_width=1 / 4).rotate_about_origin(k * 360 * DEGREES / 100) for
              k in range(100)])
        ...
        for objc, objr, objm in zip(chords_by_circumference, chords_by_radialline, chords_by_midpoint):
            ...
            temp_radialline = Line(second_circle_group[0].get_center(),
                                   second_circle_group[0].get_center() + rad * temp_radialline, color=GREEN)
...
class SimpleProblem(MovingCameraScene):
    def construct(self):
        ...
        for _ in range(n):
            ...
            random_radius = Line(ORIGIN, rad * RIGHT, stroke_color=random_color(),
                                 stroke_width=DEFAULT_STROKE_WIDTH / 2, stroke_opacity=0.25)
        ...
        probability_statements = VGroup(
            MathTex("\\mathbb{P}", "(", "\\text{random point lying inside}\\\\", "\\text{a circle of radius }", " r",
                    "<1", ")"),
            MathTex("="),
...
class CircumferenceMethod(MovingCameraScene):
    def construct(self):
        ...
        glow = VGroup(
            *[Line(ORIGIN, 1 * RIGHT / 4, color=YELLOW, stroke_width=1 / 4).rotate_about_origin(k * 360 * DEGREES / 100)
              for k in range(100)])
        ...
        center_line = Line(circle_group[0].get_bottom() + 0.25 * DOWN, circle_group[0].get_top() + 0.25 * UP,
                           color=BLUE)
        fliplist = []
        for obj in chords:
        ...
        center_angle = Angle(Line(center_line.get_center(), center_line.get_start()), center_to_end_line, radius=0.5,
                             other_angle=False)
        ...
        expected_length[1].move_to(
            expected_length[0][1].get_center() + expected_length[1].get_center() - expected_length[1][0].get_center())
        expected_length[2].move_to(
            expected_length[0][1].get_center() + expected_length[2].get_center() - expected_length[2][0].get_center())
...
class RadiallineMethod(MovingCameraScene):
    def construct(self):
        ...
        for obj in chords:
            ...
            random_radius = Line(circle_group[1].get_center(), circle_group[1].get_center() + random_radius,
                                 color=GREEN)
        ...
        new_integral = MathTex("\\mathbb{E}(L)", "=", "\\int_0^{\\pi/2}2\\sin\\theta\\cdot", "\\sin\\theta",
                               "\\,d\\theta")
        ...
        center_line = Line(circle_group[1].get_center() + rad * DOWN + DOWN / 4,
                           circle_group[1].get_center() + rad * UP + UP / 4, color=RED)
        ...
        twotheta_angle = Angle(Line(circle_group[1].get_center(), center_line.get_start()),
                               Line(circle_group[1].get_center(), vertical_chord.get_start()), radius=0.5)
        twotheta_text = MathTex("2\\theta").next_to(twotheta_angle)
        twotheta_text.set_background_stroke(width=5)
        self.play(Write(
            VGroup(tangent_line, center_line, center_dot, twotheta_line, twotheta_angle, twotheta_text, tangent_angle,
                   theta_text)), run_time=2)
        self.wait()
        ...
        self.wait()
        self.play(
            FadeOut(VGroup(vertical_chord, chords, tangent_line, center_line, center_dot, twotheta_line, twotheta_angle,
                           twotheta_text, tangent_angle, theta_text)),
...
class ExpectedValue(MovingCameraScene):
    def construct(self):
        ...
        table = VGroup(
            ...
            MathTex("\\text{Point on circle,} \\\\ \\text{line at random angle}"),
            MathTex("\\frac{4}{\\pi}\\sin^2\\theta"), MathTex("\\frac{16}{3\\pi}"),
            MathTex("\\text{Point on radial line,} \\\\ \\text{line at random angle}"),
            MathTex("\\frac{2}{\\pi}\\sin\\theta\\sinh^{-1}(\\tan\\theta)"), MathTex("\\frac{2}{\\pi}(2G+1)")
        )
        ...
        self.wait()
        table = VGroup(
            MathTex("\\text{Random method}"), MathTex("\\mathbb{E}(\\text{length})"),
            MathTex("\\mathbb{P}(\\text{pts. on opp. sides})"),
            ...
            MathTex("\\text{Point on circle,} \\\\ \\text{line at random angle}"), MathTex("\\frac{16}{3\\pi}"),
            MathTex("\\frac{1}{3}"),
            MathTex("\\text{Point on radial line,} \\\\ \\text{line at random angle}"),
            MathTex("\\frac{2}{\\pi}(2G+1)"), MathTex("\\frac{427+60\\pi^2-480\\ln 2}{180\pi^2}")
        )
        table.arrange_in_grid(6, 3)
        for k in [0, 3, 6, 9, 12, 15]:
            table[k].shift(LEFT / 2)
...
class MidpointMethod(MovingCameraScene):
    def construct(self):
        ...
        def get_chord_by_midpoint(circle):
            ...
            first_point = circle_center + r * RIGHT + semichord * UP
            second_point = circle_center + r * RIGHT + semichord * DOWN
            chord = Line(first_point, second_point).rotate(2 * PI * np.random.random(),
                                                           about_point=circle_group[2].get_center())
            if chord.get_length() >= rad * np.sqrt(3):
                chord.color = BLUE
            else:
                chord.color = WHITE
        ...
        horizontal_radius = Line(circle_group[2].get_center(), circle_group[2].get_center() + rad * RIGHT, color=GREEN,
                                 stroke_color=GREEN)
        x = rad / 2
        ...
        new_integral = MathTex("\\mathbb{E}(L)", "=", "\\int_0^{\\pi/2}2\\sin\\theta\\cdot", "\\sin 2\\theta",
                               "\\,d\\theta")
        new_integral.scale(0.875).move_to(expected_length[0]).shift(4 * DOWN)
        self.play(Write(substitution))
        self.play(Write(new_integral))
        ...
        tangent_line = Line(vertical_chord.get_end() + 2 * LEFT, vertical_chord.get_end() + 2 * RIGHT, color=GREEN)
        center_line = Line(circle_group[2].get_center() + rad * DOWN + DOWN / 4,
                           circle_group[2].get_center() + rad * UP + UP / 4, color=RED)
        tangent_angle = Angle(tangent_line, Line(vertical_chord.get_end(), vertical_chord.get_start()), radius=0.75)
        theta_text = MathTex("\\theta").set_background_stroke(width=5)
        theta_text.next_to(tangent_angle)
        theta_text.shift(UP / 4)
        ...
        self.play(
            Write(VGroup(tangent_line, center_line, center_dot, tangent_angle, theta_text, theta_distribution, box)),
            run_time=2)
        self.wait()
        self.play(
...
class RandomPointRandomAngle3(MovingCameraScene):
    def construct(self):
        ...
        def get_chord_by_randompointrandomangle(circle, randomness=True):
            ...
            horizontal_line = Line(circle_group[3].get_center(), circle_group[3].get_center() + rad * RIGHT,
                                   color=GREEN, stroke_width=0.875)
        ...
        expected_length = VGroup(
            MathTex("\\mathbb{E}(L)", "=",
                    "\\int_0^{\\pi/2}\\int_0^1 2\\sqrt{1-(r\\cos t)^2}\\cdot 2r\\,dr\\frac{dt}{\\pi/2}"),
        ...
        self.play(
            FadeOut(
                VGroup(illustrative_glow, horizontal_brace, illustrative_tangle, chord_length, smallx, illustrative_t,
                       illustrative_chord)),
            FadeIn(vertical_chord)
        )
        self.wait()
        ...
        tangent_line = Line(vertical_chord.get_end() + 2 * LEFT, vertical_chord.get_end() + 2 * RIGHT, color=GREEN)
        center_line = Line(circle_group[3].get_center() + rad * DOWN + DOWN / 4,
                           circle_group[3].get_center() + rad * UP + UP / 4, color=RED)
        tangent_angle = Angle(tangent_line, Line(vertical_chord.get_end(), vertical_chord.get_start()), radius=0.75)
        theta_text = MathTex("\\theta").set_background_stroke(width=5)
        theta_text.next_to(tangent_angle)
        theta_text.shift(UP / 4)
...
class RandomRadialRandomAngle2(MovingCameraScene):
    def construct(self):
        ...
        def get_chord_by_randompointrandomangle(circle, randomness=True):
            ...
            perpendicular_chord = Line(first_point, second_point, stroke_width=0.875)
            chord_group.add(perpendicular_chord)
            horizontal_line = Line(circle_group[4].get_center(), circle_group[4].get_center() + rad * RIGHT,
                                   color=GREEN, stroke_width=0.875)
            chord_group.add(horizontal_line)
            random_angle = 2 * PI * np.random.random() if randomness else 45 * DEGREES
            r = r * np.cos(random_angle)
        ...
        expected_length = VGroup(
            MathTex("\\mathbb{E}(L)", "=",
                    "\\int_0^{\\pi/2}\\int_0^1 2\\sqrt{1-(x\\cos t)^2}\\cdot \\,dx\\frac{dt}{\\pi/2}"),
        ...
        tangent_line = Line(vertical_chord.get_end() + 2 * LEFT, vertical_chord.get_end() + 2 * RIGHT, color=GREEN)
        center_line = Line(circle_group[4].get_center() + rad * DOWN + DOWN / 4,
                           circle_group[4].get_center() + rad * UP + UP / 4, color=RED)
        tangent_angle = Angle(tangent_line, Line(vertical_chord.get_end(), vertical_chord.get_start()), radius=0.75)
        theta_text = MathTex("\\theta").set_background_stroke(width=5)
        theta_text.next_to(tangent_angle)
        theta_text.shift(UP / 4)
        theta_distribution = MathTex("f_{\\Theta}(\\theta)", "=",
                                     "\\frac{\\sin\\theta\\sinh^{-1}(\\tan\\theta)}{\\pi/2}")
        theta_distribution.move_to(expected_length[-1])
        theta_distribution.shift(2 * DOWN)
        self.play(
            Write(theta_distribution),
...
class CircumferenceMethodProbability1(MovingCameraScene):
    def construct(self):
        ...
        event_probability = MathTex("\\mathbb{P}(E|\\theta)", "=",
                                    "2\\left(\\frac{2\\theta-\\sin 2 \\theta}{2\\pi}\\right)\\left(1-\\frac{2\\theta-\\sin2\\theta}{2\\pi}\\right)")
        ...
        green_glow = VGroup(
            *[Line(ORIGIN, 1 * RIGHT / 4, color=GREEN, stroke_width=1 / 4).rotate_about_origin(k * 360 * DEGREES / 100)
              for k in range(100)])
        red_glow = VGroup(
            *[Line(ORIGIN, 1 * RIGHT / 4, color=RED, stroke_width=1 / 4).rotate_about_origin(k * 360 * DEGREES / 100)
              for k in range(100)])
        ...
        def get_two_points(circle):
            ...
            circle_radius = circle.radius
            temp_group = VGroup()
            temp_dot1 = Dot(circle_center + RIGHT * circle_radius * np.sqrt(np.random.random())).rotate(
                2 * PI * np.random.random(), about_point=circle_center)
            temp_dot2 = Dot(circle_center + RIGHT * circle_radius * np.sqrt(np.random.random())).rotate(
                2 * PI * np.random.random(), about_point=circle_center)
        ...
        probability_statements[1].move_to(
            probability_statements[0][1].get_center() + probability_statements[1].get_center() -
            probability_statements[1][0].get_center())
...
class RadialLineProbability(MovingCameraScene):
    def construct(self):
        ...
        random_chord = add_end_dots(random_chord)
        tangent_line = Line(circle_group[1].get_center() + rad * DOWN + LEFT,
                            circle_group[1].get_center() + rad * DOWN + RIGHT, color=TEAL)
        tangent_angle = Angle(tangent_line, random_chord[0], radius=0.5)
        theta_text = MathTex("\\theta").set_background_stroke(width=5)
        theta_text.next_to(tangent_angle)
        theta_text.shift(UP / 4)
        ...
        probability_statements[1].move_to(
            probability_statements[0][1].get_center() + probability_statements[1].get_center() -
            probability_statements[1][0].get_center())
        probability_statements[2].move_to(
            probability_statements[0][1].get_center() + probability_statements[2].get_center() -
            probability_statements[2][0].get_center())
        probability_statements[1].shift(3 * DOWN / 2)
        probability_statements[2].shift(6 * DOWN / 2)
...
class MidpointProbability(MovingCameraScene):
    def construct(self):
        ...
        random_chord = add_end_dots(random_chord)
        tangent_line = Line(circle_group[j].get_center() + rad * DOWN + LEFT,
                            circle_group[j].get_center() + rad * DOWN + RIGHT, color=TEAL)
        tangent_angle = Angle(tangent_line, random_chord[0], radius=0.5)
        theta_text = MathTex("\\theta").set_background_stroke(width=5)
        theta_text.next_to(tangent_angle)
        theta_text.shift(UP / 4)
        ...
        self.play(
            ...
            FadeOut(VGroup(event_definition, random_chord, points_group, theta_text, tangent_angle, tangent_line,
                           method_names[1]))
        )
        self.wait()
...
class RadomRadialPointRandomAngleProbability(MovingCameraScene):
    def construct(self):
        ...
        self.play(
            ...
            FadeOut(VGroup(density_function, random_chord, points_group, theta_text, tangent_angle, tangent_line,
                           method_names[j], box, probability_statements))
        )
        self.wait()
