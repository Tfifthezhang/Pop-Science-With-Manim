...
class reservoir_problem(Scene):
    ...
    def insert_image(self):
        image = SVGMobject('svg_icon/question.svg', fill_color=RED, color=BLUE).scale(0.8).shift(DOWN)
...
class reservoir_general(Scene):
    ...
    def image_insert(self):
        paper = ImageMobject('svg_icon/reservoir.png').scale(1.35)
        self.play(SpinInFromNothing(paper))
        self.play(paper.animate.move_to(2 * LEFT))
        image = ImageMobject('svg_icon/jeffrey_vitter.jpg').to_edge(2 * RIGHT)
