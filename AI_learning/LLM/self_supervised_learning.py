...
class represent_learning(Scene):
    ...
    def intro(self):  # 机器学习的任务是什么，机器学习效果依赖于好的特征
        svg_image = SVGMobject('NN.svg', fill_color=WHITE).scale(1.2)
    ...
    def feature_trans(self):
        ...
        svg_image = SVGMobject('NN.svg', fill_color=WHITE).scale(0.8).to_edge(2 * DOWN)
        self.play(Create(svg_image))
        self.wait(2)
    ...
    def nn_structure(self):
        ...
        transformer = SVGMobject('attention.svg',
                                 opacity=None,
                                 stroke_color=BLUE,
                                 stroke_opacity=0.9,
    ...
    def nn_example1(self):
        # self.NN_func
        cat = SVGMobject('cat.svg').scale(0.6).to_edge(LEFT+UP)
        dog = SVGMobject('dog.svg').scale(0.6).next_to(cat, RIGHT)
    ...
    def represent_clf(self):
        ...
        # 输出y
        svg_clf = SVGMobject('logistic.svg', fill_color=WHITE).scale(0.6).next_to(self.NN_func[-1], RIGHT)
...
class unsupervised_example(Scene):
    ...
    def unsupervised_intro(self):
        ...
        svg_image = SVGMobject('文档.svg', fill_color=WHITE).scale(0.9).next_to(unsup_en, 2 * DOWN)
...
class supervised_example(ThreeDScene):
    ...
    def supervised_intro(self):
        ...
        l_label_svg = VGroup(
            *[SVGMobject('文档_split.svg', fill_color=i).scale(0.6) for i in [BLUE, RED, GREEN]])
    ...
    def write_text_before_animate(self):
        ...
        svg_image = SVGMobject('NN.svg', fill_color=WHITE).scale(0.6).next_to(formula_x, 2 * RIGHT)
...
class llm_train(Scene):
    ...
    def sentence_example(self):
        svg_image = SVGMobject('文档.svg', fill_color=WHITE).scale(0.9).to_edge(LEFT+2*UP)
    ...
    def BERT_train(self):
        ...
        svg_image = SVGMobject('NN.svg', fill_color=WHITE).scale(0.8).next_to(self.NN_node, LEFT)
        self.play(FadeIn(svg_image))
...
class ssl_end(Scene):
    ...
    def represent_learning(self):
        ...
        svg_image = SVGMobject('NN.svg', fill_color=WHITE).scale(0.8).next_to(formula_x, RIGHT)
        formula_y = MathTex("f(X)").scale(2).next_to(svg_image, 2 * RIGHT)
        svg_clf = SVGMobject('logistic.svg', fill_color=WHITE).scale(0.8).next_to(formula_y, RIGHT)
